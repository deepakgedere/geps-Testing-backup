...
//TODO Constructor
    ...
    public void msaFlow() {
        try {
            ...
            JsonNode integerationStatusNode = testDataJsonNode.get("purchaseOrderRequests").get("porIntegerationStatus");
            ...
            if(type.equalsIgnoreCase("POC") || type.equalsIgnoreCase("Sales")) {
...
//TODO Step 5: Call API to Update Status
            ...
            for(int i = 0; i < integerationStatusNode.size(); i++) {
                msaFtpHelper.connectionEstablish(ftpHost, ftpPort, ftpUser, ftpPassword);
                String status = integerationStatusNode.get(i+1).asText();
                msaExcelHelper.updateExcelForOtherStatus(localExcelFilePath, status);
                msaFtpHelper.connectionEstablishAndUploadFilesForOtherStatus(ftpHost, ftpPort, ftpUser, ftpPassword, localExcelFilePath, excelUploadFilePath);
                msaApiHelper.updateStatus(readPORAPIUrl, id);
                if(i == 1) {
                    break;
                }
