        ...
        try {
            ...
            if (type.equalsIgnoreCase("SD")) {
                Response approveResponse = page.waitForResponse(
                        response -> response.url().startsWith(appUrl + "/api/PurchaseOrders/GetNonPocOrderSchedules/") && response.status() == 200,
            ...
            if (type.equalsIgnoreCase("SD")) {
                ...
                apiResponse = page.request().fetch(appUrl + "/api/PurchaseOrdersNonPoc/" + getUid, RequestOptions.create());
            } else {

                apiResponse = page.request().fetch(appUrl + "/api/PurchaseOrders/" + getUid, RequestOptions.create());
            }
