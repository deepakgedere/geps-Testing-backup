    ...
    public String downloadFile(String remotePath, String localPath, String porReferenceNumber, JsonNode jsonNode, String type) {
        try {
            ...
            if (!poProcessed) {
                ...
                String fileName = type.equalsIgnoreCase("SD") ? "_NPT1_SCM-YGS_R0.xls" : "_PT1_SCM-YGS_R0.xls";
                excelFileName = porReferenceNumber + fileName;
            } else {
                String fileName = type.equalsIgnoreCase("SD") ? "_NPT1_SCM-YGS_R1.xls" : "_PT1_SCM-YGS_R1.xls";
                excelFileName = porReferenceNumber + fileName;
            }
...
//TODO XLS File Upload
            ...
            logger.error("Exception in Connection Establish and Upload Files function: {}", exception.getMessage());
        }
        closeConnection();
    }

    public void connectionEstablishAndUploadFilesForOtherStatus(String server, int port, String user, String password, String localXLSFilePath, String remotePathXLS) {
        try {
            connectionEstablish(server, port, user, password);
...
//TODO XLS File Upload
            ...
            logger.error("Exception in Connection Establish and Upload Files for Other Status function: {}", exception.getMessage());
        }
        closeConnection();
    }
